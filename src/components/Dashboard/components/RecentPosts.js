    ...
    return(
      <React.Fragment>
        <Card style={{width: '100%', maxWidth: '100%'}}>
              ...
              <CardContent>
                <List>
                  {!postsLoading && posts.length ? posts.slice(0, 5).filter(post => post.approved || post.rejected).map(post => (
                    <ListItem key={post._id} alignItems="flex-start">
                      ...
                      <Tooltip title={ post.rejected ? 'Rejected': post.approved ? 'Approved': 'Pending'} aria-label="approved">
