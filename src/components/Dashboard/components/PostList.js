...
class PostList extends Component {
	...
	render(){
		const { classes, postSuccess, postError, postLoading, postsLoading, given, received } = this.props;
	    const { open, anchorEl, posts } = this.state;
		return(
			<React.Fragment>
				{!postsLoading && posts.length ? posts.map(post => (<Card key={post._id}>
						<CardHeader
					        avatar={
					          <Avatar alt={post.postedByName ? post.postedByName.substring(1,1): 'Image not Available'} src={given ? post.postedToPhotoURL: post.postedByPhotoURL} />
					        }
					        action={
					        	<>
						          <Tooltip title={post.rejected ? 'Rejected': post.approved ? 'Approved': 'Pending'}>
							          <IconButton color={ post.rejected ? 'secondary': post.approved ? 'primary' : ''}>
							          	<VerifiedUserIcon />
							          </IconButton>
						          </Tooltip>
						          ...
						          <Menu
							        ...
							        >
								        <MenuItem onClick={() => this.handleMenuItem('delete', post._id)}>
								        	Delete
								        </MenuItem>
							        </Menu>
						        </>
					          ...
					          secondary={
					            <React.Fragment>
					              <Typography
					                ...
					                color="textPrimary"
					              >
					                {post.positive ? post.positive: 'No comments added'}
					               </Typography>
					          ...
					          secondary={
					            <React.Fragment>
					              <Typography
					                ...
					                color="textPrimary"
					              >
					               {post.negative ? post.negative: 'No comments added'}
					               </Typography>
					            </React.Fragment>
					          }
					        />
			    ...
			    { !postsLoading && !posts.length ? <Typography variant='h1' style={{textAlign: 'center'}}>No posts found to show</Typography>: null}
			    { postsLoading && !posts.length ? <Loader />: null}
			    { postSuccess && postSuccess.size > 0 ? <CustomizedSnackbars open={true} message={postSuccess.get('message')} status={'success'} />: null}
		    </React.Fragment>
		)
	}
}
