...
class DashBoard extends Component {
  ...
  componentDidMount(){
    new firebase.auth().onAuthStateChanged(async user => {
      if(user && Array.isArray(user.providerData) && user.providerData.length) {
  ...
  render(){
    const { user, givenPosts, receivedPosts, isReceivedPost, isGivenPost} = this.state;
    const { postsLoading } = this.props;
    return (
      <React.Fragment>
        
        {postsLoading ? <Loader /> : (<div className="conatiner">
           <div className="row">
             <div className="col-lg-3 col-md-3 col-sm-12 col-xs-12">
                <List component="nav" aria-label="main mailbox folders">
                  ...
                  <ListItem button onClick={event => this.handleGivenPosts(event)}>
                    <ListItemAvatar>
                    ...
                    <ListItemSecondaryAction style={{marginRight:20}}>
                      {isGivenPost && givenPosts ? formateNumber(givenPosts.length): 0}
                    </ListItemSecondaryAction>
                  </ListItem>
                  <ListItem button onClick={event => this.handleReceivedPosts(event)}>
                    <ListItemAvatar>
                      <CallReceivedIcon color="primary" />
                    </ListItemAvatar>
                    <ListItemText
                      primary="Received Posts"
                    />
                    <ListItemSecondaryAction style={{marginRight:20}}>
                      {isReceivedPost && receivedPosts ? formateNumber(receivedPosts.length): 0}
                    </ListItemSecondaryAction>
                  </ListItem>
                </List>
              ...
              <div className="col-lg-4 col-md-3 col-sm-12 col-xs-12">   
                  { isGivenPost && givenPosts.length ? <PostList given={true} received={false} updatePosts={this.updatePosts} postsLoading={postsLoading} posts={givenPosts}/>: null}
                  { isReceivedPost && receivedPosts.length ? <PostList received={true} given={false} updatePosts={this.updatePosts} postsLoading={postsLoading} posts={receivedPosts}/>: null}
                  { isGivenPost && !givenPosts.length ? <Typography variant='h4' className="no-records">No posts Found</Typography>: null}
                  { isReceivedPost && !receivedPosts.length ? <Typography variant='h4' className="no-records">No posts Found</Typography>: null}
              </div>
