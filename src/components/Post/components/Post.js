...
class Post extends Component{
	...
	render(){
		const { classes, createPostLoading,
		    createPostError,
		    createPostSuccess,
		    user, users } = this.props;
		const { positive, negative, advice, open, preferences, newPost, errorMessage } = this.state;
		return(
			<React.Fragment>
				<Dialog
		          open={open}
		        ...
		        >
		          ...
		          <DialogContent>
				    ...
				    <Search users={users} profile={false} post={true} createPost={this.createPost}/>
				    { preferences && <>
				    	<h4>{newPost.postedByName} preferences</h4>
				    	<p>Please post your comments as per user preferences</p>
				    	<FormControlLabel
				        control={
				          preferences.positive == 'yes' ? <Checkbox checked={true} /> : <Checkbox indeterminate/>
				        }
				        label="Positive"
				      />
				      <FormControlLabel
				        control={
				          preferences.negative == 'yes' ? <Checkbox checked={true} /> : <Checkbox indeterminate/>
				        }
				        label="Negative"
				      />
				      <FormControlLabel
				        control={
				          preferences.advice == 'yes' ? <Checkbox checked={true} /> : <Checkbox indeterminate/>
				        }
				        label="Advice"
				      /></>}
				    <TextField
				          required={preferences && preferences.positive == 'yes' ? true: false}
				        ...
				        <br />
				        <TextField
				          required={preferences && preferences.negative == 'yes' ? true: false}
				          id="standard-required"
				          name="negative"
				          label="Negative"
				        ...
				        <br />
				        <TextField
				          required={preferences && preferences.advice == 'yes' ? true: false}
				          id="standard-required"
				          name="advice"
				          label="Advice"
				          defaultValue=""
		        ...
		        { createPostSuccess && createPostSuccess.size > 0 ? <CustomizedSnackbars open={true} message={createPostSuccess.get('message')} status='success' />: null}
		        { errorMessage ? <CustomizedSnackbars open={true} message={errorMessage} status='error' />: null}
			</React.Fragment>
		)
	}
}
...
const mapStateToProps = state => {
    ...
    const createPostLoading = state.getIn(['Post', 'post', 'create', 'loading'], false);
    const createPostError = state.getIn(['Post', 'post', 'create', 'errors'], Map());
    const createPostSuccess = state.getIn(['Post', 'post', 'create', 'success'], Map());
