...
class Notifications extends Component {
  ...
  render(){
    const { classes, posts, postsError, postsLoading, postUpdateSuccess,
    postUpdateError,
    postUpdateLoading } = this.props;
        ...
        <div className="conatiner">
           <div className="row">
             <div className="col-lg-3 col-md-6 col-sm-12 col-xs-12">
              ...
              <div style={{marginRight: 29, marginLeft: -35}} className="col-lg-4 col-md-4 col-sm-12 col-xs-12"> 
             {!postsLoading && posts && posts.length ? posts.map(post => (<Card key={post._id}>
                <CardHeader
                      avatar={
                      ...
                      action={
                        <>
                          ...
                          <Tooltip title="Approve">
                            <IconButton onClick={() => this.updatePost(post, 'approve')} tooltipPosition='bottom-left' color='primary'>
                              <VerifiedUserIcon />
                            </IconButton>
                          </Tooltip>
                          <Tooltip title="Reject">
                            <IconButton onClick={() => this.updatePost(post, 'reject')} tooltipPosition='bottom-left' color='secondary'>
                              <VerifiedUserIcon />
                            </IconButton>
                          </Tooltip></>}
                          {post.approved && <Chip
              ...
              </Card>)): <Typography variant='h4' style={{textAlign: 'center'}}>No posts found to show</Typography>}
            </div>
          </div>
          </div>

          { postsLoading ? <Loader />: null}
          { postUpdateSuccess && postUpdateSuccess.size > 0 ? <CustomizedSnackbars open={true} message={postUpdateSuccess.get('message')} status='success' />: null}
        </React.Fragment>
    )
  }
}
...
const mapStateToProps = state => {
    ...
    const postUpdateSuccess = state.getIn(['Dashboard', 'post', 'update', 'success'], Map());
    const postUpdateError = state.getIn(['Dashboard', 'post', 'update', 'errors'], Map());
    const postUpdateLoading = state.getIn(['Dashboard', 'post', 'update', 'loading'], false);
...
export default withRouter(connect(mapStateToProps, actionsToProps)(Notifications))