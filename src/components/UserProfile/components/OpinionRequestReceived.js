...
class OpinionRequestReceived extends Component {
    ...
    return (
      <Suspense>
        <Stepper orientation="vertical">
          {!user.opinionRequestsReceived ||
          !user.opinionRequestsReceived.length ? (
            <h5>No opinion requests received</h5>
          ) : null}
          {user.opinionRequestsReceived.map((orr, index) => (
            <Step key={orr._id} connector={<ColorlibConnector />}>
              <StepLabel
                optional={
                  <>
                    {getPastTime(orr.createdAt)}
                    {getCardSubHeaderProfileSummary(orr)}
                  </>
                }
                icon={
                  <Avatar className={classes.smallAvatar} alt={orr.userName}>
                    {this.renderReaction('follow')}
                  </Avatar>
                }
              >
                {this.renderUsername(orr)}
              </StepLabel>
...
OpinionRequestReceived.propTypes = {
...
export default withRouter(
  connect(
    mapStateToProps,
    actionsToProps,
  )(withStyles(styles)(OpinionRequestReceived)),
)
