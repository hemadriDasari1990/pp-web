...
class ProfileFolowersStepper extends Component {
      ...
      <Suspense>
        <Stepper orientation="vertical">
          {!user.followers || !user.followers.length ? (
            <h5>No followers history found</h5>
          ) : null}
          {user.followers.map((follower, index) => (
            <Step key={follower._id} connector={<ColorlibConnector />}>
              <StepLabel
                optional={
                  <>
                    {getPastTime(follower.createdAt)}
                    {getCardSubHeaderProfileSummary(follower)}
                  </>
                }
                icon={
                  <Avatar
...
ProfileFolowersStepper.propTypes = {
...
export default withRouter(
  connect(
    mapStateToProps,
    actionsToProps,
  )(withStyles(styles)(ProfileFolowersStepper)),
)
