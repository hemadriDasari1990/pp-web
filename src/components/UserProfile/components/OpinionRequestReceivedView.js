...
class OpinionRequestReceivedView extends Component {
  goBack = () => {
  ...
  render() {
    ...
    const hasOpinionRequests =
      profileUser && profileUser.opinionRequestsReceived.length ? true : false
    return (
      <React.Fragment>
        <Zoom in={true} timeout={2000}>
          <List disablePadding={true} subheader={this.renderSubHeader()}>
            {hasOpinionRequests
              ? profileUser.opinionRequestsReceived.map(f => (
              ...
              : null}
            {profileUser && !profileUser.opinionRequestsReceived.length && (
...
OpinionRequestReceivedView.propTypes = {}
...
export default withRouter(
  connect(
    mapStateToProps,
    actionsToProps,
  )(withStyles(styles)(OpinionRequestReceivedView)),
)
